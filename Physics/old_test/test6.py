...
def run_radial_model():
    ...
    plt.xlabel(r'$\log_{10}\Sigma_{\rm gas}$ (условн.)')
    plt.ylabel(r'$\log_{10}(g_{\rm DM}/g_{\rm bar})$')
    ...
    plt.colorbar(label="N points")
    plt.xlabel(r'$\log_{10}\Sigma_\star$ (L$_\odot$/pc$^2$)')
    plt.ylabel(r'$\log_{10}(g_{\rm DM}/g_{\rm bar})$')
    ...
    sigma_star_med = np.median(sigma_star)
    sigma_gas_rel = sigma_gas / sigma_gas_med
    sigma_star_rel = sigma_star / sigma_star_med
    ...
    # базовая модель g_DM ∝ Σ_*^alpha Σ_gas^beta F_BH
    # пока оцениваем α, β, γ через линрег по логарифмам
    ...
    x1 = np.log10(sigma_star[mask_reg])
    x2 = np.log10(sigma_gas[mask_reg])
    x3 = np.log10(F_BH[mask_reg])

    # регрессия: log g_DM = C + a * x1 + b * x2 + c * x3
    X = np.vstack([np.ones_like(x1), x1, x2, x3]).T
    coeffs, *_ = np.linalg.lstsq(X, log_gdm, rcond=None)
    C, alpha_hat, beta_hat, gamma_hat = coeffs

    # предсказание и метрики
    log_gdm_pred = C + alpha_hat * x1 + beta_hat * x2 + gamma_hat * x3
    ...
    print("=== РЕЗУЛЬТАТЫ РЕГРЕССИИ (лог g_DM) с учётом F_BH ===")
    print(f"log g_DM ≈ C + α log Σ_* + β log Σ_gas + γ log F_BH")
    print(f"C    = {C:.3f}")
    print(f"α    = {alpha_hat:.3f}")
    print(f"β    = {beta_hat:.3f}")
    print(f"γ    = {gamma_hat:.3f}  (ожидается < 0 для подавления осадка при большом f_bulge)")
    print(f"R²   = {R2:.3f}")
    print(f"corr = {corr:.3f}")

    # построим модель g_DM для всех точек
    log_sigma_star_all = np.log10(sigma_star)
    log_sigma_gas_all = np.log10(sigma_gas)
    log_F_BH_all = np.log10(F_BH)

    log_gdm_model_all = (
        C + alpha_hat * log_sigma_star_all +
        beta_hat * log_sigma_gas_all +
    ...
    plt.plot([lim_min, lim_max], [lim_min, lim_max],
             "r--", label="model = obs")
    plt.xlabel(r'$\log_{10} g_{\rm DM}^{\rm model}$')
    plt.ylabel(r'$\log_{10} g_{\rm DM}^{\rm obs}$')
    plt.title("Сравнение двухпараметрической резервуарной модели с F_BH")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
    ...
    delta_log = log_obs - log_mod

    plt.figure(figsize=(7, 5))
    plt.scatter(np.log10(sigma_star[valid_corr]), delta_log, s=5, alpha=0.4)
    plt.axhline(0.0, color="k", linestyle="--")
    plt.xlabel(r'$\log_{10}\Sigma_\star$')
    plt.ylabel(r'$\Delta \log_{10} g_{\rm DM}$ (obs - model)')
    plt.title("Остатки резервуарной модели vs log Σ_* (с учётом F_BH)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
    ...
    plt.axhline(0.0, color="k", linestyle="--")
    plt.xlabel(r'$\log_{10}\Sigma_{\rm gas}$')
    plt.ylabel(r'$\Delta \log_{10} g_{\rm DM}$ (obs - model)')
    plt.title("Остатки резервуарной модели vs log Σ_gas (с учётом F_BH)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
